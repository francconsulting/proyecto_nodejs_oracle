...
fs.readFile("querys/tdc_pdtes_CDS.sql", { encoding: "utf-8" }, (err, data) => {
  ssql = data;
});
...
var tdcs = (err, conn) => {
  let configExcel = {
    tipo: "stream",
    name_wb:
      "Hoja de trabajo-TDC'S Pendientes con operaciones_" +
      reloj.getDate() +
      ".xlsx",
    name_ws: "datos V"
  };
  ...
  mensaje = "Conectado. Esperando a ejecutar la consulta CDS_INFORMES.....";
...
var job2 = new CronJob({
  ...
  onComplete: function () {
    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Finalización  del proceso CDS_INFORMES" + reloj.getDate());
  },
  start: true,
...
job2.start();
console.log("job2 corriendo: ", job2.running);
